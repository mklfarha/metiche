...
func (m *module) List(ctx context.Context,
	...
	// cache hit to do it.
	cacheKey := list.CacheKey("ListDecisionToken", query, args)
	if !skipShared {
	...
	runList := func() (any, error) {
		...
		if err != nil {
			...
			m.logger.Error("error in executing query for ListDecisionToken", zap.String("query", query), zap.Int("args", len(args)), zap.Error(err))
			return types.ListResponse{}, err
		}
		defer rows.Close()
		var scanGetters []func(*repogen.DecisionToken) any
		...
		var items []repogen.DecisionToken
		for rows.Next() {
			var i repogen.DecisionToken
		...
		return types.ListResponse{
			DecisionToken: mapModelsToEntities(items),
...
var listFieldRegistry = map[string]func(*repogen.DecisionToken) any{

	"id": func(i *repogen.DecisionToken) any { return &i.ID },

	"decision_uuid": func(i *repogen.DecisionToken) any { return &i.DecisionUUID },

	"team_uuid": func(i *repogen.DecisionToken) any { return &i.TeamUUID },

	"project_uuid": func(i *repogen.DecisionToken) any { return &i.ProjectUUID },

	"token": func(i *repogen.DecisionToken) any { return &i.Token },

	"weight": func(i *repogen.DecisionToken) any { return &i.Weight },

	"created_at": func(i *repogen.DecisionToken) any { return &i.CreatedAt },

	"updated_at": func(i *repogen.DecisionToken) any { return &i.UpdatedAt },
}
