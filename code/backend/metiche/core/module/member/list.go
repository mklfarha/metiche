...
func (m *module) List(ctx context.Context,
	...
	// cache hit to do it.
	cacheKey := list.CacheKey("ListMember", query, args)
	if !skipShared {
	...
	runList := func() (any, error) {
		...
		if err != nil {
			...
			m.logger.Error("error in executing query for ListMember", zap.String("query", query), zap.Int("args", len(args)), zap.Error(err))
			return types.ListResponse{}, err
		}
		defer rows.Close()
		var scanGetters []func(*repogen.Member) any
		...
		var items []repogen.Member
		for rows.Next() {
			var i repogen.Member
		...
		return types.ListResponse{
			Member:      mapModelsToEntities(items),
...
var listFieldRegistry = map[string]func(*repogen.Member) any{
	...
	"id": func(i *repogen.Member) any { return &i.ID },

	"team_uuid": func(i *repogen.Member) any { return &i.TeamUUID },

	"key": func(i *repogen.Member) any { return &i.Key },

	"display_name": func(i *repogen.Member) any { return &i.DisplayName },

	"role": func(i *repogen.Member) any { return &i.Role },

	"last_seen_at": func(i *repogen.Member) any { return &i.LastSeenAt },

	"status": func(i *repogen.Member) any { return &i.Status },
