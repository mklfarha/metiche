...
func (m *module) List(ctx context.Context,
	...
	// cache hit to do it.
	cacheKey := list.CacheKey("ListAgent", query, args)
	if !skipShared {
	...
	runList := func() (any, error) {
		...
		if err != nil {
			...
			m.logger.Error("error in executing query for ListAgent", zap.String("query", query), zap.Int("args", len(args)), zap.Error(err))
			return types.ListResponse{}, err
		}
		defer rows.Close()
		var scanGetters []func(*repogen.Agent) any
		...
		return types.ListResponse{
			Agent:       mapModelsToEntities(items),
...
var listFieldRegistry = map[string]func(*repogen.Agent) any{
	...
	"status": func(i *repogen.Agent) any { return &i.Status },

	"last_seen_at": func(i *repogen.Agent) any { return &i.LastSeenAt },
