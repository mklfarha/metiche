...
func (m *module) List(ctx context.Context,
	...
	// cache hit to do it.
	cacheKey := list.CacheKey("ListBoardLoginLink", query, args)
	if !skipShared {
	...
	runList := func() (any, error) {
		...
		if err != nil {
			...
			m.logger.Error("error in executing query for ListBoardLoginLink", zap.String("query", query), zap.Int("args", len(args)), zap.Error(err))
			return types.ListResponse{}, err
		}
		defer rows.Close()
		var scanGetters []func(*repogen.BoardLoginLink) any
		...
		var items []repogen.BoardLoginLink
		for rows.Next() {
			var i repogen.BoardLoginLink
		...
		return types.ListResponse{
			BoardLoginLink: mapModelsToEntities(items),
...
var listFieldRegistry = map[string]func(*repogen.BoardLoginLink) any{

	"id": func(i *repogen.BoardLoginLink) any { return &i.ID },

	"account_uuid": func(i *repogen.BoardLoginLink) any { return &i.AccountUUID },

	"agent_uuid": func(i *repogen.BoardLoginLink) any { return &i.AgentUUID },

	"secret_hash": func(i *repogen.BoardLoginLink) any { return &i.SecretHash },

	"redirect_path": func(i *repogen.BoardLoginLink) any { return &i.RedirectPath },

	"requested_via": func(i *repogen.BoardLoginLink) any { return &i.RequestedVia },

	"expires_at": func(i *repogen.BoardLoginLink) any { return &i.ExpiresAt },

	"consumed_at": func(i *repogen.BoardLoginLink) any { return &i.ConsumedAt },

	"created_at": func(i *repogen.BoardLoginLink) any { return &i.CreatedAt },

	"updated_at": func(i *repogen.BoardLoginLink) any { return &i.UpdatedAt },
}
