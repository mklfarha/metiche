...
func (m *module) List(ctx context.Context,
	...
	// cache hit to do it.
	cacheKey := list.CacheKey("ListIntentToken", query, args)
	if !skipShared {
	...
	runList := func() (any, error) {
		...
		if err != nil {
			...
			m.logger.Error("error in executing query for ListIntentToken", zap.String("query", query), zap.Int("args", len(args)), zap.Error(err))
			return types.ListResponse{}, err
		}
		defer rows.Close()
		var scanGetters []func(*repogen.IntentToken) any
		...
		var items []repogen.IntentToken
		for rows.Next() {
			var i repogen.IntentToken
		...
		return types.ListResponse{
			IntentToken: mapModelsToEntities(items),
...
var listFieldRegistry = map[string]func(*repogen.IntentToken) any{

	"id": func(i *repogen.IntentToken) any { return &i.ID },

	"intent_uuid": func(i *repogen.IntentToken) any { return &i.IntentUUID },

	"team_uuid": func(i *repogen.IntentToken) any { return &i.TeamUUID },

	"project_uuid": func(i *repogen.IntentToken) any { return &i.ProjectUUID },

	"token": func(i *repogen.IntentToken) any { return &i.Token },

	"weight": func(i *repogen.IntentToken) any { return &i.Weight },

	"created_at": func(i *repogen.IntentToken) any { return &i.CreatedAt },

	"updated_at": func(i *repogen.IntentToken) any { return &i.UpdatedAt },
}
