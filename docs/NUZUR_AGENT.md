...
| `apply` | live | classify the **live** `information_schema`, require its columns to be exactly the model's (`create.sql`), `CREATE OR REPLACE` every view with its columns in the **model's order**, drop stale ones, then run `check-live` |
| `check-live` | live | the daily drift check. It verifies, without selecting any row value: the live schema is classified and has exactly the model's columns; `metiche_nuzur` holds exactly one view per table with definer `nuzur_views@localhost` and `SQL SECURITY DEFINER`; each view's column names equal the model's, **in the model's order** (a view in any other order fails, naming the view and the first differing position); `COUNT(*)` of non-NULL values is 0 for every redacted column; and both accounts' grants, lock state and `MAX_USER_CONNECTIONS` are exactly as expected. |

**Column order follows the model, not the table.** Every view lists its columns in the
order of `code/backend/metiche/core/repository/sql/schema/create.sql`, the model as
generated by nuzur, and a redacted column keeps its model position as a typed NULL. The
live table's physical order can differ: `deploy/sql/*.sql` migrations add columns with
`ALTER TABLE … ADD COLUMN … AFTER x`, so production's `agent.token_hash` sits after
`client_key` while the model has it after `account_uuid`. The nuzur data manager compares
the views with the model, so the physical order would show up there as a column-order
mismatch. The live modes read the model from the checkout (`NUZUR_SCHEMA` overrides it),
and they fail closed if the live column set and the model's differ in either direction,
naming each `table.column`: nothing is silently dropped or added.
...
1. Read `information_schema.columns` for `metiche` (names, data types) and the model's `create.sql` (names, order).
2. **Fail before any DDL** if any of these hold, printing each offender:
   - a column in the model but not in the live table, or live but not in the model;
...
3. For each table, with its columns in the model's order, emit
...
**`deploy/sql/nuzur/test-views-docker.sh`**, on throwaway MySQL **8.4.11 and 8.0.46**: 48/48
...
- `SELECT *` works on all 27 views. 0 of 9 canaries are visible, and every positive-control
  marker is. All 403 column names and orders match the model (`create.sql`).
...
- Column order: with `agent.token_hash` and `session.parent_session_uuid` moved physically,
  as `ALTER … AFTER` leaves them, `apply` still gives every view the model's order, checked
  as `nuzur_ro` against `create.sql`. A view recreated by hand in the physical order fails
  `check-live`, naming it, and `apply` fixes it. A model with an extra column, or missing a
  live column, is refused before DDL, naming the column. With the reordering line reverted,
  8 checks fail.
